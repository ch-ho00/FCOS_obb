    ...
    def _resize_img(self, results):
        if self.keep_ratio:
            ...
            img, w_scale, h_scale = mmcv.imresize(
                results['img'], results['scale'], return_scale=True)
#        print("???????????????",results['img_prefix'], flush=True)
#        annotation = results['gt_bboxes'].shape[-1]
#        assert annotation == 5 or annotation == 4 
        if 'dota' not in results['img_prefix']:
...
#        assert annotation == 5 or annotation == 4
        ...
        for key in results.get('bbox_fields', []):
            ...
            if 'dota' not in results['img_prefix'] :
                ...
                #print("here2",results[key][:5], results['scale_factor'],flush=True)
                bboxes = results[key] * results['scale_factor']
                bboxes[:,:4][:, 0::2] = np.clip(bboxes[:,:4][:, 0::2], 0, img_shape[1])
                bboxes[:,:4][:, 1::2] = np.clip(bboxes[:,:4][:, 1::2], 0, img_shape[0])
...
class RandomFlip(object):
    ...
    def bbox_flip(self, bboxes, img_shape, direction):
        ...
        annotation = 4 if bboxes.shape[-1] % 4 == 0 else 5
        flipped = bboxes.copy()
        if direction == 'horizontal':
            w = img_shape[1]
            flipped[..., 0::annotation] = w - bboxes[..., 2::annotation]
            if annotation == 5:   
                flipped[..., 4::annotation] = -flipped[..., 4::annotation]
        elif direction == 'vertical':
            h = img_shape[0]
            flipped[..., 1::annotation] = h - bboxes[..., 3::annotation]
        else:
