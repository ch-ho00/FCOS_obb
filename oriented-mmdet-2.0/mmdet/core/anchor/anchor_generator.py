...
class AnchorGenerator(object):
    ...
    def gen_single_level_base_anchors(self,
        ...
        if angles is not None:  
            ...
            hs, anchor_angles = self._meshgrid(hs, angles) 
            base_anchors = [
            ...
            w = ws.round()
            h = hs.round()
            x_ctr = (base_anchors[:, 0] + 0.5 * w).round()
            y_ctr = (base_anchors[:, 1] + 0.5 * h).round()
            angle = base_anchors[:, 4]
            base_anchors = torch.stack([x_ctr, y_ctr, w, h, angle], dim=-1)
        else:
            base_anchors = [
