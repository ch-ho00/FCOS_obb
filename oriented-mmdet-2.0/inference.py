...
class DetectorModel():
    ...
    def gt_single_vis(self, detections, srcpath, dstpath, slide_size, chip_size):
#    detections = self.inference_single(srcpath, slide_size, chip_size)
        img = draw_poly_detections(srcpath, detections, self.classnames, scale=1, threshold=0.3)
        print(dstpath)
        cv2.imwrite(dstpath, img)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--config",nargs='?',const="iou_th_1e1",type=str)
    parser.add_argument("--model",nargs='?',const="obb/epoch_2.pth",type=str)
    parser.add_argument("--img",nargs='?',const="P0000.jpg",type=str)
    parser.add_argument("--gt",nargs='?',const=0,type=int)

    args = parser.parse_args()

    dirs_ = os.listdir('./inference_test/')
    dirs_ = [dir_ for dir_ in dirs_ if '.png' in dir_ ]
    roitransformer = DetectorModel(args.config, args.model)

    if 1:
        for img in dirs_:
            roitransformer.inference_single_vis(r'inference_test/%s'%(img),
                                                r'inference_test/result/0526_%s_%s_detected.jpg'%(args.config,img[:-4]),
                                                (512, 512),
                                                (1024, 1024))
    else:
        with open('./DOTA_trainval1024_w_obb.json') as json_file:
            data = json.load(json_file)
            fn2imgid = { img_['file_name'] : img_['id'] for img_ in data['images']}
            for img in dirs_:
                img_id = int(fn2imgid[img])
                detections = {}
                for gt_ann in data['annotations']:
                    if gt_ann['image_id'] == img_id:
                        one = np.array([1.])
                        app = gt_ann['segmentation']
                        app = np.array([np.concatenate((i,one)) for i in app])
                        try:
                            detections[gt_ann['category_id']] = np.concatenate((detections[gt_ann['category_id']], app)) 
                        except:
                            detections[gt_ann['category_id']] = app
             #       elif int(gt_ann['image_id']) > fn2imgid[img]:
             #           break
                total_detections = [np.zeros((0, 9)) for _ in range(15)]
                if len(detections) >0 :
                    for key in detections:
                        total_detections[key-1] = detections[key]
                    for i,arr in enumerate(total_detections):
                        if arr.shape[0] == 0:
                            total_detections[i] = np.array([[0.]*9])
                    roitransformer.gt_single_vis(total_detections,
                                   r'inference_test/%s'%(img),
                                   r'inference_test/result/0526_%s_%s_detected.jpg'%(args.config,img[:-4]),
                                   (512, 512),
                              (1024, 1024))
