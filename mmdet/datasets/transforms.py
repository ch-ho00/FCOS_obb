...
class BboxTransform(object):
    ...
    def __init__(self, max_num_gts=None,
        ...
        self.scale_test = scale_test
    def __call__(self, bboxes, img_shape, scale_factor, flip=False,rand_scale=None):
        if bboxes.shape[-1] == 5:
            bboxes[:,:-1] = bboxes[:,:-1] * scale_factor
            if flip:
                bboxes = bbox_flip(bboxes, img_shape)
            bboxes[:,:-1][:, 0::2] = np.clip(bboxes[:,:-1][:, 0::2], 0, img_shape[1] - 1)
            ...
            if self.max_num_gts is None:
                ...
                padded_bboxes = np.zeros((self.max_num_gts, 5), dtype=np.float32)
...
class MaskTransform(object):
    ...
    def __init__(self,
                 scale_test= None):
        self.scale_test = scale_test
    def __call__(self, masks, pad_shape, scale_factor, flip=False,rand_scale=None):
        ...
        if self.scale_test != None:
            if self.scale_test == -1:
                #print('before:::::',masks[0].shape,rand_scale,flush=True)
                masks = [ mmcv.imrescale(mask,rand_scale,interpolation='nearest') for mask in masks]
                #print("after ::::::",masks[0].shape,flush=True)
                masks = [ mmcv.impad(mask,pad_shape[:2],pad_val=0) for mask in masks]
            else:
                assert self.scale_test > 0
                masks = [ mmcv.imrescale(mask,self.scale_test,interpolation='nearest') for mask in masks]
                masks = [ mmcv.impad(mask,pad_shape[:2],pad_val=0) for mask in masks] 
