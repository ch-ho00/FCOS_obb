...
class DOTA_Seg_Dataset(CustomDataset):
    ...
    def load_annotations(self, ann_dir):
        #img_ids = os.listdir(ann_dir)
        self.img_prefix = "./data/dota"

        with open('./data/dota/dota_img_dims.txt','r') as f:
            tmp = f.read()
        tmp = [ row.split(',') for row in  tmp.split('\n')]
        #      print(tmp[:3],flush=True)
        dims_ = {}
        for i,row in enumerate(tmp):
            try:
                dims_[row[2]] = [int(row[0]), int(row[1])]
            except:
                continue
        self.img_dims = dims_

        with open(ann_dir+'/annotation.json','r') as f:
            img_infos=json.load(f)
        img_infos= [ {a: np.array(dic[a]) for a in dic}  for dic in img_infos]
        for img in img_infos:
            img['filename'] = str(img['filename'])
            [img['height'], img['width']] = dims_[img['filename']]
            img['filename'] = str(img['filename']).replace('labelTxt','images')
        return img_infos
    ...
    def _filter_imgs(self, min_size=32):
        ...
        valid_inds = []
        for i, row in enumerate(self.img_infos):
            if min([row['height'],row['width']]) >= min_size:
                valid_inds.append(i)
#        valid_inds = valid_inds[:20]
        return valid_inds
    ...
    def _parse_ann_info(self, ann_info, with_mask=True):
        ...
        for i, ann in enumerate(ann_info):
            for j in range(ann['bbox'].shape[0]):
                x1, y1, w, h = ann['bbox'][j]
                if self.debug:
                    total+=1
                ...
                if ann['iscrowd'][j]:
                    ...
                    gt_labels.append(self.cat2label[ann['category_id'][j]])
                if with_mask:
                    gt_masks.append(ann['segmentation'][j])
                    mask_polys = [
                        p for p in ann['segmentation'][j] if len(p) >= 6
    ...
    def prepare_train_img(self, idx):
        ...
        img = mmcv.imread("./data/dota" + img_info['filename'])
        # corruption
        ...
        if self.with_mask:
            gt_masks = self.mask_transform(ann['masks'], pad_shape,
                                              scale_factor, flip)
        # convert to 5-data pt represetnation
        gt_bboxes = backward_convert(gt_bboxes)
        gt_bboxes_ignore = backward_convert(gt_bboxes_ignore)
    ...
    def polar_target_single(self, gt_bboxes, gt_masks, gt_labels, points, regress_ranges):
        ...
        areas = ((gt_bboxes[:, 2] -gt_bboxes[:, 0])**2 + (gt_bboxes[:, 3] -gt_bboxes[:, 1])**2)**0.5 * (
            (gt_bboxes[:, 4] -gt_bboxes[:, 2])**2 + (gt_bboxes[:, 5] -gt_bboxes[:, 3])**2)**0.5
        ...
        gt_bboxes_x = gt_bboxes[None][:,:,::2]
        gt_bboxes_y = gt_bboxes[None][:,:,1::2]
        #xs ys 分别是points的x y坐标
        xs, ys = points[:, 0], points[:, 1]
        xs = xs[:, None].expand(num_points, num_gts)
        ys = ys[:, None].expand(num_points, num_gts)
        left = xs - (gt_bboxes[..., 0]+ gt_bboxes[...,])
