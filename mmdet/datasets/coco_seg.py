...
class Coco_Seg_Dataset(CustomDataset):
    ...
    def prepare_train_img(self, idx):
        ...
        if self.scale_test == None or self.scale_test != -1:
            img, img_shape, pad_shape, scale_factor = self.img_transform(img, img_scale, flip, keep_ratio=self.resize_keep_ratio)
        else:
            img, img_shape, pad_shape, scale_factor,rand_scale = self.img_transform(img, img_scale, flip, keep_ratio=self.resize_keep_ratio)
        ...
        if self.proposals is not None:
            if self.scale_test == None or self.scale_test != -1:        
                ...
                proposals = self.bbox_transform(proposals, img_shape, scale_factor,flip,rand_scale)
        ...
        if self.with_crowd:
            if self.scale_test == None or self.scale_test != -1:
                ...
                gt_bboxes_ignore = self.bbox_transform(gt_bboxes_ignore, img_shape,scale_factor,flip,rand_scale)

        if self.with_mask:
            if self.scale_test == None or self.scale_test != -1:
                gt_masks = self.mask_transform(ann['masks'], pad_shape, scale_factor, flip)
            else:
                gt_masks = self.mask_transform(ann['masks'], pad_shape,scale_factor,flip,rand_scale)
        ...
        #print(len(gt_bboxes), gt_labels.shape, gt_masks.shape,len(gt_masks),img.shape,"!!!!",flush=True)
       # utils.viz.plot_bbox(np.transpose(img,(1,2,0)),gt_bboxes,scores=None,labels=gt_labels)
    ...
    def polar_target_single(self, gt_bboxes, gt_masks, gt_labels, points, regress_ranges):
        ...
        for mask in gt_masks:
            ...
            #print(contour.shape, flush=True)
            #exit()
            y, x = cnt
