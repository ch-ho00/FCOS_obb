...
def inference_detector(model, imgs,scale=None):
    ...
    img_transform = ImageTransform(
        size_divisor=cfg.data.test.size_divisor,scale_test=scale, **cfg.img_norm_cfg)
...
def show_result(img,
    ...
    if scale != None:
        ...
        img = mmcv.impad(img,pad_shape,pad_val=0)
    img = img.copy()
...
def show_result_pyplot(img,
    ...
    """
    img = show_result(
        img, result, class_names, score_thr=score_thr, show=False,scale=scale)
