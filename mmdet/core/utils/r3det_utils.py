...
import numpy as np 
import cv2 
from mmdet.core.bbox import rbbox_overlaps_cy, rbbox_overlaps_torch
...
def anchor_target_layer(gt_boxes_h, gt_boxes_r, anchors,gt_labels, iou_p_th, iou_n_th, gpu_id=0):
    ...
    gt_boxes_r = (N,5) 
    '''
    anchor_states = torch.zeros((anchors.shape[0],)).cuda()
    labels = torch.zeros((anchors.shape[0],)).long().cuda()
    if gt_boxes_r.shape[0]:
        # [N, M]
        overlaps = rbbox_overlaps_torch(anchors,gt_boxes_r)
        ...
        delta_theta = torch.abs(target_boxes[:, -1] - anchors[:, -1])
    ...
    return labels, target_delta, anchor_states, target_boxes
