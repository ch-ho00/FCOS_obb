...
import random
import torch
def dbbox2mask(dboxes):
    polys = RotBox2Polys(dboxes)
    masks = []

    width = 1024; height=1024
    for i in range(polys.shape[0]):
        img = Image.new('L', (width, height), 0)
        ImageDraw.Draw(img).polygon(polys[i], outline=1, fill=1)
        mask = np.array(img)
        masks.append(mask)
...
def distance2obb(points, distance,max_shape= None):
    ...
    Args:
        points = (N,2)
        distance = (n,5) distance from given points to 4 boundaries (top,right,bottom, left) with angle
    Returns
        Tensor: Decoded bboxes (n,5)  : (x_ct, y_ct, w, h ,theta)
    '''
    distance[:,4] = distance[:,4] * (np.pi/2) + np.pi *5 /4 
    thetas = 630 - ((distance[:,4] %(np.pi *2))* 180 / np.pi)
    thetas = thetas * np.pi / 180
    wl = torch.max(distance[:,1] , distance[:,3])
    ws = torch.min(distance[:,1] , distance[:,3])

    hl = torch.max(distance[:,0] , distance[:,2])
    hs = torch.min(distance[:,0] , distance[:,2])

    dx =  torch.cos(thetas) * ((wl-ws)/2) - ((hl-hs)/2) * torch.sin(thetas)
    dy = torch.cos(thetas) * ((hl-hs)/2) +  torch.sin(thetas) * ((wl-ws)/2)


    pm_x = (distance[:,1] > distance[:,3]).int() ; pm_x[pm_x==0] = -1; pm_x = pm_x.float()
    pm_y = (distance[:,0] > distance[:,2]).int() ; pm_y[pm_y==0] = -1; pm_y = pm_y.float()

    x_ct = points[:,0] + dx * pm_x
    y_ct = points[:,1] + dy * pm_y
    h = distance[:,0] + distance[:,2]
    w = distance[:,1] + distance[:,3]
    if max_shape is not None:
        x_ct = x_ct.clamp(min=0,max=max_shape[1] - 1)
        y_ct = y_ct.clamp(min=0,max=max_shape[1] - 1)
    # order of h,w is opposite as distance of the ground truth is > 3/2 pi
    return torch.stack([x_ct,y_ct,h,w,distance[:,4]],-1)
...
def mask2poly_single(binary_mask):
    ...
    try:
       ...
       # max_contour = contours[max_id]
        ...
        print(binary_mask.shape, e,flush=True)
        exit()
#        import pdb
#        pdb.set_trace()
    return poly
...
def gt_mask_bp_obbs(gt_masks,  with_module=True):
    ...
    gt_polys = mask2poly_single(gt_masks)
    gt_bp_polys = get_best_begin_point_warp_single(gt_polys)
...
#    print('rotbox2polys',y_ctr[:5] , ss[:5] , (-w[:5] / 2.0) , cs[:5] , (h[:5] / 2.0),'\n\n------------',flush=True)
...
def RotBox2Polys_torch_order(dboxes):
    """
