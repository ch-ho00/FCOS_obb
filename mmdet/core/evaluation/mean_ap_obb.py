...
def get_cls_results(det_results, gt_bboxes, gt_labels, gt_ignore, class_id):
  ...
  #  print( [ len(det) for det in det_results],[len(det[class_id]) for det in det_results], [len(det[class_id][0]) for det in det_results ],flush=True )
    cls_dets =np.array( [det[class_id] for det in det_results ] ) # det bboxes of this class
   # cls_dets = np.vstack(cls_dets)
    ...
    cls_gt_ignore = np.array(cls_gt_ignore)
...
def eval_map(det_results,
    ...
    det_results = np.array([ det[0] for det in det_results ])
    num_classes = 15  # positive class num
