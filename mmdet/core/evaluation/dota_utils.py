...
def OBBDetComp4(dataset, results):
    ...
    for idx in range(len(dataset)):
        filename = dataset.img_infos[idx]['filename']
        result = results[idx]
       # print(type(results),len(results),type(result),len(result),type(result[0]),len(result[0]),type(result[0][0]),len(result[0][0]),flush=True)
        for label in range(len(result)):
            rbboxes = result[label]
            # import pdb
            # pdb.set_trace()
            ...
            if cls_name not in results_dict:
                results_dict[cls_name] = []
            for i in range(rbboxes.shape[0]):
                poly =  np.around(np.around(rbboxes[i][:-1]),decimals=1)
                score = np.around(rbboxes[i][-1],decimals=12)
                outline = filename + ' ' + str(score) + ' ' + ' '.join(map(str, poly))
                results_dict[cls_name].append(outline)
    return results_dict
