...
#        bbox_pred = scale_bbox(self.polar_reg(reg_feat)).float().exp()
        ...
        return cls_score,  centerness, mask_pred # originally 2nd output = bbox_pred
     ...
     #              bbox_preds,
       ...
       # assert len(cls_scores) == len(bbox_preds)
        ...
        mlvl_points = self.get_points(featmap_sizes, mask_preds[0].dtype,
                                      mask_preds[0].device) #originally bbox_preds
        result_list = []
...
#        print(scale_factor,flush=True)
        ...
        mlvl_masks = []
        mlvl_centerness = []
        for cls_score,  mask_pred, centerness, points in zip( #bbox_pred
                cls_scores, mask_preds, centernesses, mlvl_points): # bbox_preds
