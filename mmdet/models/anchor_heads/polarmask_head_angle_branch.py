...
from mmdet.core import dbbox2mask, RotBox2Polys,distance2bbox, force_fp32, multi_apply, multiclass_nms_rbbox,distance2obb, merge_rotate_aug_bboxes #, multiclass_nms, multiclass_nms_with_mask
...
#       normal_init(self.polar_mask, std=0.01)
        ...
        normal_init(self.angle_reg,std=0.01)
...
#        mask_feat = x
        ...
        angle_pred = self.Sigmoid(self.angle_reg(reg_feat)) * pi/2 
        angle_pred = angle_pred + torch.ones_like(angle_pred) * pi *1.25
        bbox_pred = torch.cat([bbox_pred, angle_pred],1)
        
        #bbox_pred[:,4,:,:] = F.(bbox_pred[:,4,:,:])
...
#            self.bbox2vis(np.transpose(img[1].detach().cpu().numpy(),(1,2,0)),gt_bboxes[1],"gt_bboxes2.png")
           ...
           # centerness weighted iou loss
            loss_bbox = self.loss_bbox(
                pos_decoded_bbox_preds,
                pos_decoded_bbox_targets,
#                cls_score=flatten_cls_scores[pos_inds].detach(),
#                gt_cls = flatten_labels[pos_inds].detach(),
                weight=pos_centerness_targets,
                avg_factor=pos_centerness_targets.sum())
