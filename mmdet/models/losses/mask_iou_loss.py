...
class MaskIOULoss(nn.Module):
    ...
    def forward(self, pred, target, weight, avg_factor=None):
        ...
        total = torch.stack([pred[:,:-1],target[:,:-1]], -1)
        l_max = total.max(dim=2)[0]
        l_min = total.min(dim=2)[0]
        angle_diff = ((torch.abs(pred[:,-1] - target[:,-1]))*2/np.pi*weight).sum()/avg_factor
        loss = (l_max.sum(dim=1) / l_min.sum(dim=1)).log()
        #print(loss,angle_diff,flush=True)
        loss = (loss * weight)
        loss = loss.sum() / avg_factor
        #print(loss,angle_diff,flush=True)
        loss = loss + angle_diff
        return loss
